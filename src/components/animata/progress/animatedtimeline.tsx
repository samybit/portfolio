...
const TimelineItem: React.FC<TimelineItemProps> = ({
  ...
  const lineFillStyle: React.CSSProperties = {
    ...
    transformOrigin: reverseDirection ? "bottom" : "top",
  ...
  return (
    ...
    >
      <div className="hidden md:flex relative me-4 flex-col items-center pt-3">
        <div
      ...
      <div className={cn("grow", !isLast && "mb-6")}>
        {customRender ? (
          customRender(event)
        ) : (
          <>
  ...
  return (
    ...
    <div className={cn("relative", className)} onMouseLeave={() => setActive(null)}>
      {events.map((event, index) => {
        const isDotActive = state.active !== null && (reverseDirection ? index >= state.active : index <= state.active);
        const isLineActive = state.active !== null && (reverseDirection ? index >= state.active : index < state.active);
        ...
        if (reverseDirection) {
          ...
          const revIndex = lastIndex - index;
          const revLineIndex = lastIndex - index - 1;
          const revActive = state.active !== null ? lastIndex - state.active : null;
          const revPrev = state.prev !== null ? lastIndex - state.prev : null;
          
          dotDelay = computeDelays(revIndex, revActive, revPrev).dotDelay;
          lineDelay = computeDelays(revLineIndex, revActive, revPrev).lineDelay;
        } else {
          const delays = computeDelays(index, state.active, state.prev);
          dotDelay = delays.dotDelay;
          lineDelay = delays.lineDelay;
        }
